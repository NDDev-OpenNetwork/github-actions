...
# A silent assertion is not a diagnosis: the b22 smoke failed twice with
# "exit status 1: no diagnostic output" before this trap named the line.
report_error() {
  local rc="$1"
  local line="$2"
  local command="$3"
  printf >&2 'smoke failed at line %s: %s (exit %s)\n' "${line}" "${command}" "${rc}"
  exit "${rc}"
}
