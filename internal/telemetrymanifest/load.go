...
func Load(path string) (Manifest, error) {
	...
	if err != nil {
		return Manifest{}, fmt.Errorf("open telemetry manifest: %w", err)
	}
	defer file.Close()
...
func Decode(reader io.Reader) (Manifest, error) {
	...
	if err != nil {
		return Manifest{}, fmt.Errorf("read telemetry manifest: %w", err)
	}
	if len(data) > maxManifestBytes {
		return Manifest{}, fmt.Errorf("telemetry manifest exceeds %d bytes", maxManifestBytes)
	}
