...
func testConfig(t *testing.T) Incus {
	...
	return Incus{
		...
		CacheClaimFile:        filepath.Join(filepath.Dir(cert), "cache-claims.json"),
		CacheClaimLockFile:    filepath.Join(filepath.Dir(cert), "cache-claims.lock"),
...
func TestIncusConfigRejectsUnsafeOrAmbiguousValues(t *testing.T) {
	tests := []struct {
		...
		{"cache claim equals queue intent", func(c *Incus) { c.CacheClaimFile = c.QueueIntentFile }, "must be distinct"},
		{"missing cache claim parent", func(c *Incus) { c.CacheClaimFile = "/missing/cache-claims.json" }, "invalid cache_claim_file"},
